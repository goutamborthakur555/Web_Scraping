...
source = url.urlopen('https://www.yelp.com/search?find_desc=train%20museum&find_loc=LA') # Enter the link to be scraped
category_filter = "Museums" # Enter the Category
...
for main in mains:
    ...
    try:
        business_reviews.append(main.find("span", {"class": "text__09f24__1RhSS reviewCount__09f24__EUXPN text-color--black-extra-light__09f24__2ZRGr text-align--left__09f24__ceIWW"}).text)
    except:
...
result.to_excel("Yelp_File_Raw.xlsx", columns = header, index = False)

filtered_result = result[result.business_category.str.contains(category_filter)]
filtered_result.to_excel("Yelp_File_Filtered.xlsx", columns = header, index = False)
